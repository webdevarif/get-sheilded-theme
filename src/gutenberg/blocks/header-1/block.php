<?php
/**
 * Header 1 Block - PHP Registration
 * 
...
/**
 * Register Header 1 Block
 */
function gst_register_header_1_block() {
    // Check if Gutenberg is available
    if (!function_exists('register_block_type')) {
        return;
    }
    
    // Debug: Log that we're registering the block
    error_log('Registering Header 1 block...');

    // Register the block with a simple editor
    register_block_type('gst/header-1', array(
        'api_version' => 2,
        'attributes' => array(
            'desktopLogo' => array(
                'type' => 'object',
            ...
            'navigationItems' => array(
                'type' => 'array',
                'default' => array(
                    array('label' => 'Home', 'url' => '#home', 'active' => true),
                    array('label' => 'About', 'url' => '#about', 'active' => false),
                    array('label' => 'Services', 'url' => '#services', 'active' => false),
                    array('label' => 'Work', 'url' => '#work', 'active' => false),
                    array('label' => 'Blog', 'url' => '#blog', 'active' => false),
                ),
            ),
            'ctaText' => array(
                'type' => 'string',
                'default' => 'Get 30min consultation',
            ),
            'ctaUrl' => array(
                'type' => 'string',
                'default' => '#contact',
        ...
        'render_callback' => 'gst_render_header_1_block',
        'editor_script' => 'gst-blocks-editor',
        'editor_style' => 'gst-blocks-editor-style',
        'style' => 'gst-blocks-style',
        'edit' => 'gst_edit_header_1_block',
    ));
    
    // Debug: Log successful registration
    error_log('Header 1 block registered successfully!');
}
// Commented out - using JavaScript registration instead
// add_action('init', 'gst_register_header_1_block');

/**
 * Edit Header 1 Block (Simple editor)
 */
function gst_edit_header_1_block($attributes) {
    ...
    $navigation_items = $attributes['navigationItems'] ?? array();
    ...
    ob_start();
    ?>
    <div class="gst-header-1-editor" style="background-color: <?php echo esc_attr($background_color); ?>; color: <?php echo esc_attr($text_color); ?>; padding: 20px; border: 2px dashed #ccc; margin: 10px 0;">
        <h3><?php echo gst_t('header_1_block'); ?></h3>
        <p><strong><?php echo gst_t('logo_text'); ?>:</strong> <?php echo esc_html($logo_text); ?></p>
        <p><strong><?php echo gst_t('cta_text'); ?>:</strong> <?php echo esc_html($cta_text); ?></p>
        <p><strong><?php echo gst_t('navigation_items'); ?>:</strong> <?php echo count($navigation_items); ?></p>
        <p><em><?php echo gst_t('configure_block'); ?></em></p>
    </div>
    <?php
    return ob_get_clean();
}
...
/**
 * Render Header 1 Block
 */
function gst_render_header_1_block($attributes) {
    ...
    ob_start();
    ?>
    <div class="gst-header-1 relative w-full z-50" 
      ...
      <div class="bg-black py-1 px-5 text-xs text-gray-400">
        <span class="font-normal tracking-wide"><?php echo gst_t('homepage'); ?></span>
      </div>
        ...
        <div class="flex items-center justify-between px-5 h-[70px] max-w-7xl mx-auto">
          ...
          <div class="flex-none hidden md:flex items-center gap-4">
            ...
            <?php
                ...
                if (function_exists('gst_language_switcher')) {
                    echo gst_language_switcher();
                }
              ?>
