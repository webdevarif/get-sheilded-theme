...
function gst_posted_on() {
	...
	$posted_on = sprintf(
		esc_html_x( 'Posted on %s', 'post date', 'get-sheilded-theme' ),
 ...
 * @return void
 */
function gst_posted_by() {
	$byline = sprintf(
		esc_html_x( ' by %s', 'post author', 'get-sheilded-theme' ),
 ...
 * @return bool|string
 */
function gst_the_excerpt( $trim_character_count = 0 ) {
	$post_ID = get_the_ID();
 ...
 */
function gst_excerpt_more( $more = '' ) {

	if ( ! is_single() ) {
		$more = sprintf( '<a class="gst-read-more text-white mt-3 btn btn-info" href="%1$s">%2$s</a>',
			get_permalink( get_the_ID() ),
			__( 'Read more', 'get-sheilded-theme' )
		);
	}
...
function gst_pagination() {
	...
	printf( '<nav class="gst-pagination clearfix">%s</nav>', wp_kses( paginate_links( $args ), $allowed_tags ) );
}
 ...
 */
function gst_the_post_pagination( $current_page_no, $posts_per_page, $article_query, $first_page_url, $last_page_url, bool $is_query_param_structure = true ) {
	...
	<div class="mt-0 md:mt-10 mb-10 lg:my-5 flex items-center justify-end posts-navigation">
		...
		if ( 1 !== $current_page_no && ! empty( $first_page_url ) ) {
			printf( '<a class="first-pagination-link btn border border-secondary mr-2" href="%1$s" title="first-pagination-link">%2$s</a>', esc_url( $first_page_url ), __( 'First', 'get-sheilded-theme' ) );
		}
		...
		echo paginate_links( [
			...
			'total'     => $total_pages,
			'prev_text' => __( 'Prev', 'get-sheilded-theme' ),
			'next_text' => __( 'Next', 'get-sheilded-theme' ),
		] );

		// Last Page
		if ( $current_page_no < $total_pages && !empty( $last_page_url ) ) {
			printf( '<a class="last-pagination-link btn border border-secondary ml-2" href="%1$s" title="last-pagination-link">%2$s</a>', esc_url( $last_page_url ), __( 'Last', 'get-sheilded-theme' ) );
		}
		?>
	</div>
	<?php
 ...
 */
function gst_is_uploaded_via_wp_admin( $gravatar_url ) {
 ...
 * @return bool
 */
function gst_has_gravatar( $user_email ) {

	$gravatar_url = get_avatar_url( $user_email );

	if ( gst_is_uploaded_via_wp_admin( $gravatar_url ) ) {
		return true;
	}
